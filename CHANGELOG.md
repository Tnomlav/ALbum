...
## v1.2.71 - 2026-09-25（查重补上"同一张图存了两份"）

- 你在"一般通過ホロリスR"里看到的重复**不是字节相同**：同一作品 `illust_149789152` 在 09-18 和 09-24 各归档了一次，尺寸完全相同、文件大小只差 0.05%（PNG 里的 tag/时间不同），所以纯字节查重按定义找不出来。实机核对：两批各 5 个文件，大小分别是 1516874 / 1516168、1601501 / 1600795、1499772 / 1499066、1573332 / 1572626、1479602 / 1478896（每对差 600~800 字节）。
- 查重因此加了一条**针对性快通道**：先用元数据（宽×高完全相同 + 文件大小相差 ≤1%）挑出"可能是同一张图存了两份"的极少数候选，**只对这些候选**解码算 dHash，再用"汉明距离 ≤6 且平均色差 ≤30"确认成组。其他图片、尺寸不同的图、大小差很多的图都不会被解码，所以默认查重仍然是秒级，不会回到"几分钟"。
- 单测补到 9 项：新增"同一作品归档两次 → 两个文件都进候选""同尺寸但大小差很多 → 不候选""同大小但尺寸不同（缩放副本）→ 不候选""尺寸未知不参与匹配"，以及 dHash 位布局与匹配阈值。
- 说明：解码仍是 ≤128px，结果写进 `cleanup_analysis_cache.txt`（签名升到 fp4，会重建一次指纹缓存）；**缩放副本（尺寸不同）仍不覆盖**——如需覆盖，可以按同样"元数据筛选 + 少量解码"的思路再加一条按比例的通道。
- 真机：1.2.71 已装到测试机；验证时手机被切到别的 App（有人在使用），端到端结果这轮没确认，下次跑一次"查重"看那一批文件是否成组。

## v1.2.70 - 2026-09-25（只保留查重：移除"相似图片"与整套 dHash 代码）

- 按你的要求去掉"相似图片"：重复图片页现在只有 **查重** 一个按钮；配套的 dHash 指纹、解码扫描、`aspectCandidatePositions`/`looksLikeSamePicture`、指纹缓存条目以及相关测试全部删除（净减约 200 行）。查重算法不变——按大小筛候选 → 读 16 KB 头签名 → 逐字节比较、遇差异即停。
- 分析缓存只保留 `head:` 条目：旧版本留下的 `sha:` / `fingerprint:` / `quick:` 条目会在下一次扫描保存缓存时自动清掉。
- 测试换成 `DuplicateScanTest`（5 项）：逐字节比较的"完全相同 / 中段有一处不同 / 长度不同 / 空文件"，以及"只有大小重复的文件才进入候选"（含未知大小不参与匹配）。
- 真机：1.2.70 已装，重复图片页只剩"查重"；人工放的两个字节相同副本在约 1–3 秒内被报为"1 组重复图片"，随后测试图已从手机与媒体库删除。

## v1.2.69 - 2026-09-25（查重换成系统清理工具那套快扫；相似图片单独按需）

- **查重换成"手机管家"式做法**：先按 MediaStore 已知的**文件大小**筛出可能重复的候选（这一步不读文件），再只读每个候选的**前 16 KB** 做廉价签名，只有签名相同的那几个文件才真正两两**逐字节比较**，而且一有差异立刻停止。不再对全库做 SHA-256、也不再解码任何图片——之前"几分钟跑不完"就是因为这两件事。
- **真机实测**（本机 2.4 万张图库、4.8 万个候选）：查重从"几分钟没结果"降到 **约 1–3 秒**；我人工放了两个字节相同的副本，扫描精确报出"1 组重复图片"并在列表里列出两个文件名。
- **相似图片单独成按钮**：能找出"重编码/缩放后肉眼相同"的 dHash 匹配需要解码候选图，属于慢速操作，所以从默认查重里拆出来，做成旁边独立的 **相似图片** 按钮（显示"正在比对图片 N/M"）；两个按钮的结果都写进同一张列表。
- 查重时显示 **正在核对文件 N/M**，随时可点"停止"，离开页面立即取消（上一版的取消/进度机制继续沿用）。
- 分析缓存（廉价签名 + 指纹）从"按条写 SharedPreferences"改为 `cleanup_analysis_cache.txt`（一次缓冲读入，只写回仍在库里的条目）；取消扫描时也会保存已算出的部分，下次接着用。

## v1.2.67 - 2026-09-25（查重有进度也能停、解码提速、回收站空状态文案）

- **查重不再是黑盒**：`findDuplicateGroups` 现在回报进度（哈希阶段 + 解码阶段的完成数/总数），"重复图片"页显示"正在分析 N/M"并画一条进度条；扫描期间原来的"全盘查重"按钮变成可点的**停止**，所以"是在跑还是卡住了"一眼就能答、也能立刻中止。
- **退出不再卡**：扫描的每个循环都插入取消检查，离开页面或点"停止"会马上停下，而不用等整个大库解码完（之前循环里没有挂起点，取消信号要等全部跑完才生效，表现就是退出卡住、CPU 还被占满）。
- **提速**：指纹解码从 ≤256px 降到 ≤128px（dHash 只要 64×64），指纹缓存签名加了版本后缀（解码尺寸变了，旧缓存值不能混用），因此下次扫描会重建一次指纹缓存，之后直接读缓存。
- **回收站空状态文案缩短**：说明由"回收站保存在应用私有目录，保留期限在设置中调整；卸载 Album 或清除应用数据会删除其中仍在的文件。"缩短为"保留期限在设置中调整；卸载或清除数据会清空回收站。"；列表为空时只保留"回收站为空"一条，标题栏里那句"没有可还原的项目"精简为"暂无内容"。
- 真机：1.2.67 已装到测试机；但操作期间手机正被使用（界面停在别的 App），这一轮的真机复核没做成，下轮补上（重点确认进度条、停止按钮与退出是否立即返回）。
