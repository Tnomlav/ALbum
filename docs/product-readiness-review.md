...
待做：4 小窗多任务隐藏——`FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS` 方案已评估后放弃：该标志只作用于新建任务，要生效就得结束当前持有播放器与小窗的 Activity，"小窗还在播放但任务消失"会连带丢掉播放器实例；改用"点最近任务卡片回到应用＝自动收起小窗并恢复全屏"的行为兜底（`Media3VideoPlayer` 的 ON_RESUME 分支），并用 `setExcludeFromRecents` 在支持它的机型上继续隐藏卡片。另：v1.2.59 归档页的遍历耗时长行还没在真机读到数字（uiautomator 对该页返回旧 dump），下轮用截图/日志再确认一次；v1.2.61（R-18 屏蔽 / 视频设置同步 / 播放器菜单横屏居中）因为测试机断连未做真机确认，需要补一次。
