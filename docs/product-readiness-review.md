...
已完成：P 页归档文件夹过滤改为预计算小写集合＋哈希查找（v1.2.50）；小窗四角"固定哪两条边"抽成 miniWindowPinnedEdges() 并由单元测试钉死四种情形（v1.2.51）；小窗期间隐藏多任务卡片尝试 setExcludeFromRecents 并调整调用时机（v1.2.52），真机结论是 OriginOS 不采纳该标志（dumpsys 里仍是 Recent #1）；裁剪页黑边层与裁剪框共用一套命中判定 cropHandleAt()（v1.2.53）；裁剪框不再被倾斜预览永久压小，角度回正后恢复原尺寸、导出也不再被内缩两次（v1.2.53）；冷启动不再在主线程解析媒体快照，实测冷启动 706ms → 232ms（v1.2.56）；缩略图缓存加命中率计数与审计日志（v1.2.56）；lint baseline 失效条目清理＋死代码扫描（v1.2.56）；APK 体积审计写入 docs/release-size.md（v1.2.56）；三个列表页"首个可见项"从每 80ms 的嵌套线性扫描改成 remember 出来的 key 哈希表（v1.2.57）；裁剪框几何按旋转后的真实边界（四角判定）重做：自动内接、移动受限、拖动缩放恢复响应（v1.2.58）；裁剪页真机脚本化检查 scripts/device-smoke.py（截图量裁剪框、倾斜内缩、回正恢复、黑边抓边），并在 CI 里每周定时跑模拟器；P 页图库遍历改为"每目录一次查询 + 目录指纹复用未变子树"，单测断言第二次遍历只列根目录（v1.2.59，耗时长行的真机读数待下轮确认）；P 页归档文件夹过滤改为预计算小写集合＋哈希查找（v1.2.50）。

待做：4 小窗多任务隐藏——`FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS` 方案已评估后放弃：该标志只作用于新建任务，要生效就得结束当前持有播放器与小窗的 Activity，"小窗还在播放但任务消失"会连带丢掉播放器实例；改用"点最近任务卡片回到应用＝自动收起小窗并恢复全屏"的行为兜底（`Media3VideoPlayer` 的 ON_RESUME 分支），并用 `setExcludeFromRecents` 在支持它的机型上继续隐藏卡片。另：v1.2.59 归档页的遍历耗时长行还没在真机读到数字（uiautomator 对该页返回旧 dump），下轮用截图/日志再确认一次；v1.2.61 里播放器菜单的横屏居中还没做真机复核，v1.2.62 的 R-18 补扫也还缺一条真实带 R-18 tag 的归档记录做端到端验证（设置页与重命名项已在真机确认）；v1.2.66 的查重端到端验证被"扫描中途手机被切到别的 App"打断，且首次全盘扫描在大库上偏慢（下一轮改用缩略图管线 + 缓存复用）。
