...
private val APP_ENGLISH = mapOf(
    ...
    "回收站保存在应用私有目录：保留期结束后自动清空，卸载或清除应用数据会同时删除其中文件" to "Trash lives in the app's private storage: it is emptied when the retention period ends, and uninstalling Album or clearing its data deletes whatever is still inside",
    "导出应用数据" to "Export app data",
    "收藏、队列、排除文件夹与偏好设置；不含媒体、缩略图和 Pixiv 登录信息" to "Favorites, queues, excluded folders and preferences; no media, thumbnails or Pixiv sign-in data",
    "导入应用数据" to "Import app data",
    "从备份文件恢复，会覆盖当前的收藏、队列与偏好设置" to "Restore from a backup file; replaces the current favorites, queues and preferences",
